...
import torch.nn.functional as F
import sys
sys.path.append('/mnt/HDD4/jineel/bowel_project/team_oxygen/model_source/contrail')
...
#             print(self.enc.load_state_dict(sd,strict=False))
    ...
    def forward(self, x):
        ...
        if self.mask_head:
            ...
            masks1 = F.interpolate(masks1,size=384,mode='bilinear')
            
            masks2 = self.mask_head_3(encs[-2])
            masks2 = F.interpolate(masks2,size=384,mode='bilinear')
